...
hourlyWeatherWrapper.removeChild(hourlyWeatherWrapper.children[0]);
  ...
  const removeInvalidLocation = () => {
    ...
    //export
    return {
      showApp, loadApp, drawWeatherData, drawInvalidLocation, removeInvalidLocation
    }
})();
...
const LOCALSTORAGE = (() => {
  ...
  return {
    save, get, remove, getSavedCities, saveCurrentCity, getCurrentCityName, getCurrentCity,findObjectByKey
  }
})();
...
const SAVEDCITIES = (() => {
  ...
  document.addEventListener('click',function(event){
    if(event.target.classList.contains('set-city')){
      ...
      WEATHER.getWeather(savedCities[cityIndex].city,false);
    }
  });
...
const GETLOCATION = (() => {
  ...
  const addCityBtn = document.querySelector('#add-city-btn');
    ...
    const _addCity = () => {
      //Make all locations only start with a capital
      str = locationInput.value.toLowerCase().split(' ');
      for (var i = 0; i < str.length; i++) {
        str[i] = str[i].charAt(0).toUpperCase() + str[i].slice(1);
      }
      location = str.join(' ');
      //The code in comment below is what you can use if you don't care about location capitals
      ...
      for(let x = 0; x<cities.length;x++){
        tempcities.push(cities[x].city);
      }
      if (tempcities.includes(location)){
...
const NEWS = (() => {
  const newsApiKey = 'd5a21385ad6e4b4c995c47fb4969baea';
})();
...
const WEATHER = (() => {
  ...
  const getWeather = (location, save) => {
      ...
      let indexOfCity = LOCALSTORAGE.findObjectByKey(cities, 'city', location);
      console.log("This is the indexOfCity city: " + indexOfCity.city);
      if(tempcities.includes(indexOfCity.city)){
         let darkskyURL = _getDarkSkyURL(indexOfCity.lat,indexOfCity.lng);
         _getDarkSkyData(darkskyURL, location);
        console.log("Went this route because city already exists");
        return
      }else{
        console.log("went this route cause city doesn't exist");
                  ...
                  .then((res) => {
                    ...
                    if(save){
                      ...
                      //  LOCALSTORAGE.save(location);
                        LOCALSTORAGE.saveCurrentCity({city:location, lat:lat, lng:lng});
...
window.onload = () => {
  ...
  let indexOfCity = LOCALSTORAGE.findObjectByKey(cities, 'city', city.city);
  if (cities.length !== 0){
    cities.forEach((city1)=>{
      //  SAVEDCITIES.drawCity(city1.city);
        SAVEDCITIES.drawCity(city1.city);
    });

    if(Object.keys(indexOfCity).length === 0 && indexOfCity.constructor === Object){
      LOCALSTORAGE.saveCurrentCity(cities[cities.length - 1]);
      WEATHER.getWeather(cities[cities.length - 1].city,false);
    }else{
      WEATHER.getWeather(city.city,false);
    }
    //
  }else{
    UI.showApp();
...
let day = currentTime.getDate();
  ...
  let year = currentTime.getFullYear();
    ...
    str += day + ' ' + monthNames[monthIndex] + ' ' + hours + ":" + minutes + ":" + seconds + " ";
    if(hours > 11){
        str += "PM"
    } else {
        str += "AM"
...
const pullData = () => {
  ...
  console.log('completed interval and repulled data on ' + timestamp);
...
//The setInterval method allows you to run a function every x amount of time.
//You can use this for a digital clock
